...
int main()
{
    ...
    Mhz = mhz_full(1,1);              /* Estimate the clock frequency */
/* $end mountainmain */
    ...
    for (stride = MAXSTRIDE - 1; stride >= 1; stride -= STRIDESTRIDE) 
      printf("s%d\t", stride);
    printf("\n");
...
#if 1    /* use hi-res cycle counters */
    cycles = fcyc2(test, elems, stride, 0);  /* call test(elems,stride) */
#else
    cycles = fcyc2_tod(test, elems, stride, 0);  /* call test(elems,stride) */
#endif
